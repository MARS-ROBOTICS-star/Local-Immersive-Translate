...
function normalizeBaseUrl(url: string) {
  return (url || "http://127.0.0.1:8765/zotero").replace(/\/+$/, "");
}

function getAuthToken() {
  if (isLocalBackendEnabled()) {
    return getPref("localBackendToken") || getPref("authkey") || "";
  }
  return getPref("authkey") || "";
}

function getBaseUrl() {
  return normalizeBaseUrl(getPref("localBackendUrl"));
}
