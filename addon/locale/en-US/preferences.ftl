...
pref-local-backend-settings = Local BabelDOC
...
pref-local-backend-url = Local service URL
pref-local-backend-autostart =
    .label = Start local BabelDOC automatically before testing or translation
...
pref-local-backend-project-dir = Project directory
pref-local-backend-uv-path = uv path
pref-local-model-configs = Model API configuration
pref-local-model-configs-description = Fill in only the models you actually use. The plugin sends the selected model's configuration to the local BabelDOC backend for each translation; empty fields fall back to the local server config file.
